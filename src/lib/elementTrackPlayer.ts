...
/**
 * The part of `ElementTrackPlayer` that `audioPlaybackManager` actually uses.
 *
 * Derived with `Pick` rather than written out, so it cannot drift from the
 * class: adding a member here that the class does not have fails to compile,
 * and removing one from the class breaks every fake that claimed it. The
 * manager takes this instead of the concrete class so its queue and feed
 * bookkeeping can be tested — jsdom has no media stack worth driving, and
 * WebKit's behaviour is the whole reason verses run on a media element at all.
 */
export type TrackPlayer = Pick<
  ElementTrackPlayer,
  | 'onEnded'
  | 'onExternalPause'
  | 'onExternalPlay'
