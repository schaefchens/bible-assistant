...
import { ElementTrackPlayer, type TrackPlayer } from './elementTrackPlayer';
...
class AudioPlaybackManager {
  ...
  private interjection: TrackPlayer;
   ...
   * `erasableSyntaxOnly` is on, so `constructor(private x: T)` does not
   * compile in this codebase.
   */
  constructor(makePlayer: (label: string) => TrackPlayer = (l) => new ElementTrackPlayer(l)) {
    this.player = makePlayer('verse');
    this.interjection = makePlayer('reply');
