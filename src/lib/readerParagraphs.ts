 ...
 *
 * Both guillemets are listed because the two conventions disagree about which
 * one closes: German sets »…« (so `«` is the closing mark, and HFA and S00 use
 * it 7,880 times between them), while Swiss/French sets «…» . Only `»` was here
 * originally, which is the mark that can never end a quotation in the two texts
 * that actually use guillemets — so a verse ending quoted speech
 * ("»Licht soll entstehen!«") did not count as closing a sentence and the
 * paragraph ran on. LUT was unaffected: it sets „…“ and `“` was already here. */
