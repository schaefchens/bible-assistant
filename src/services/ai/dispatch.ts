 ...
 *
 * To add a tool, touch two files: declare it in its domain module under
 * `tools/` (an args entry + a definition, plus that module's read-tool list if
 * its effect is audio), then add one entry here. `ToolName` is derived from
 * those args maps, so the union updates itself — which is what makes the
 * missing entry here a compile error rather than a tool the model can call and
 * nothing answers.
 */
const TOOL_REGISTRY: { [N in ToolName]: ToolHandler<N> } = {
  ...
  list_shelves: () => handleListShelves(),
  create_shelf: (args) => handleCreateShelf(args),
  update_shelf: (args) => handleUpdateShelf(args),
  delete_shelf: (args) => handleDeleteShelf(args),
  share_shelf: (args) => handleShareShelf(args),
  decide_reader: (args) => handleDecideReader(args),
  write_piece: (args) => handleWritePiece(args),
  publish_piece: (args) => handlePublishPiece(args),
  delete_piece: (args) => handleDeletePiece(args),
  add_to_shelf: (args) => handleAddToShelf(args),
  remove_from_shelf: (args) => handleRemoveFromShelf(args),
  read_shelf: (args) => handleReadShelf(args),
  read_new: (args) => handleReadNew(args),
  unfollow_shelf: (args) => handleUnfollowShelf(args),
  copy_from_shelf: (args) => handleCopyFromShelf(args),
