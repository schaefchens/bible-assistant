 ...
 * `random_passage`, and a space name that fails to resolve is **never** a
...
export function systemPrompt(locale: 'en' | 'de', translation: Translation): string {
  ...
  if (locale === 'de') {
    return [
      ...
      `Räume ("spaces") sind die eigenen Texte des Benutzers und die Texte von Menschen, die er liest — kein Bibeltext. "write_post" hält Diktiertes als ENTWURF fest: gib seine Worte weiter, nur um Satzzeichen und Absätze ergänzt (Leerzeile zwischen Absätzen), erfinde nichts dazu und schreibe niemals einen Beitrag für ihn. Sage nie, etwas sei geteilt oder veröffentlicht — das Teilen macht der Benutzer selbst in der App. "read_space" liest einen Raum vor, "read_new" alles Neue von allen (scope "today" für die Heute-Räume); danach GIB KEINE Textantwort zurück, so wie bei read_verses. Ein Raum gehört einer Person und wird deshalb meist nach ihr benannt: "lies Christophs Heute", "lies mir Annas Gedanken vor", auch nur "lies Christoph". Nenne read_space genau das, was der Benutzer gesagt hat — Namen und alles; die App löst auch den Autorennamen auf. NAMEN VON MENSCHEN SIND KEINE BIBELBÜCHER: wenn eine Lese-Bitte eine Person oder einen Raum nennt, ist es NIE read_verses. Findet read_space nichts, nennt es die vorhandenen Räume — frag dann kurz nach oder rufe "list_spaces" auf, aber weiche NIEMALS auf eine Bibelstelle aus. In einen eigenen Raum lassen sich auch Lesepläne und Tafeln teilen: "share_plan" und "share_board" veröffentlichen eine MOMENTAUFNAHME — spätere Änderungen erreichen die Leser erst, wenn der Benutzer erneut teilt, und das darfst du nicht anders darstellen. Hat der Benutzer mehrere eigene Räume und nennt keinen, frag kurz nach, statt einen zu raten.`,
  ...
  return [
    ...
    `Spaces are the user's own writing, and the writing of people they read — not scripture. "write_post" saves dictation as a DRAFT: pass their words through, edited only for punctuation and paragraphs (blank line between paragraphs), invent nothing, and never compose a piece on their behalf. Never say something has been shared or published — sharing is an act the user performs in the app. "read_space" reads one space aloud and "read_new" everything new from everyone (scope "today" for their Today spaces); after either, return NO text answer, exactly as with read_verses. A space belongs to a person and is usually named after them — "read Christoph's Today", "read me Anna's reflections", even just "read Christoph". Pass read_space exactly what the user said, name and all; the app matches the author's name too. PEOPLE'S NAMES ARE NOT BOOKS OF THE BIBLE: if a request to read names a person or a space, it is NEVER read_verses. When read_space finds nothing it names the spaces that do exist — ask which one they meant, or call "list_spaces", but NEVER fall back to a Bible passage. Reading plans and boards can be shared into the user's own spaces too: "share_plan" and "share_board" publish a SNAPSHOT — later edits reach readers only when the user shares again, and you must not describe it otherwise. If they have several spaces and name none, ask which rather than guessing.`,
