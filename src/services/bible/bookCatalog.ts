...
/**
 * What a book is called, in the reader's language.
 *
 * One line, and it was written out at nine call sites — the picker, the chat
 * verse panel (three times), the ribbon bar, the spoken heading, a reading-list
 * entry, and twice inside this file. The catalog owns the two name fields, so
 * it owns the choice between them; a tenth copy is how a third language starts
 * being half-supported.
 *
 * Takes a `BookEntry` rather than an id because the callers that can be handed
 * an unknown book each want a *different* fallback — `?12`, `Book 12`, or the
 * empty string — and none of them wants this to invent one.
 */
export function bookName(book: BookEntry, locale: Locale): string {
  return locale === 'de' ? book.nameDe : book.nameEn;
}
