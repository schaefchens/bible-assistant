...
function findSpacePost(spaceId: string, postId: string): Post | null {
  const state = snapshot();
 ...
 * doubt while reading.
 */
function subscribedPosts(state: SpaceSnapshot = snapshot()): LocatedPost[] {
...
function findPostById(postId: string): LocatedPost | null {
  const state = snapshot();
