...
type TickerChunk = { words: string[]; activeIdx: number };

/**
 * Exported for `tests/component/eyesFreeTicker.test.tsx`, which is the only
 * caller outside this file. Its gap-bridging is a render-behaviour rule — the
 * chunk survives a silent moment but not a stop — and rendering the whole
 * eyes-free overlay to reach it would test the gestures and the speech
 * synthesis instead.
 */
export function RollingTicker() {
  ...
  /** The chunk the current word falls in, or `null` during a gap. */
  const live = useMemo<TickerChunk | null>(() => {
    if (!isVerse || words.length === 0 || wordIndex < 0) return null;
    const chunkStart = Math.floor(wordIndex / CHUNK_SIZE) * CHUNK_SIZE;
    return {
      words: words.slice(chunkStart, chunkStart + CHUNK_SIZE),
      activeIdx: wordIndex - chunkStart,
    };
  }, [isVerse, words, wordIndex]);

  /**
   * The chunk to keep showing through a brief gap — punctuation, the moment
   * between two tracks, a chapter announcement — so the ticker doesn't
   * flicker. Dropped when playback fully stops.
   *
   * **State adjusted during render, guarded — not a ref.** A `useMemo` that
   * both read and wrote `lastChunkRef` is what this replaces: it made the
   * ticker's output depend on how many times it had rendered rather than on
   * its inputs, which is why `react-hooks/refs` rejects it and why it is
   * genuinely wrong under a double render. An effect is not the alternative
   * either: `set-state-in-effect` is an error here too, and an effect would
   * paint the stale chunk for a frame before correcting it. Same move
   * `AppShell` uses for its pending indicator.
   *
   * `live` is memoized, so its identity is stable while its inputs are and
   * each branch settles in at most one extra render — one per *spoken word*,
   * since `currentWordIndex` is the only input that moves during playback.
   */
  const [held, setHeld] = useState<TickerChunk | null>(null);
  if (stopped) {
    if (held !== null) setHeld(null);
  } else if (live !== null && live !== held) {
    setHeld(live);
  }

  const display = stopped ? null : (live ?? held);
