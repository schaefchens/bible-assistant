...
import { bookName, getBookById } from '@/services/bible/bookCatalog';
...
export function ReaderPanel({ message, selected, onSelect }: Props) {
  ...
  const contextTarget = useMemo(() => {
    ...
    return {
      ...
      label: `${name} ${first.chapter}`,
    };
  }, [verses, message.headingWholeChapter, lang]);
    ...
    <>
      ...
      >
        {runs.map((run, ri) => {
          ...
          const name = book ? bookName(book, lang) : `Book ${run.bookId}`;
          const showVerseScope = message.headingWholeChapter === false;
          let headerLabel = `${name} ${run.chapter}`;
          if (showVerseScope && run.items.length > 0) {
            ...
            const versePart = ranges
              ...
              .join(listSep);
            headerLabel = `${name} ${run.chapter}${chapterSep}${versePart}`;
          }
