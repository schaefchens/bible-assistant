...
// directory — a missing include is a 500 with display_errors off, which says
// nothing by itself.
cpSync('public/api', join(root, 'api'), { recursive: true });
...
try {
  ...
  // The handlers in api/ are includes, not endpoints, and this server is PHP's
  // CLI server — which ignores .htaccess entirely, exactly like the E2E suite's.
  // So what is asserted here is the in-code guard, on the server config where
  // it is the only thing standing. bootstrap.php is the one that mattered:
  // fetched directly it used to die on an undefined APP_ROOT and print the
  // server's filesystem path, because it never runs api.php's display_errors=0.
  await check('a handler fetched directly says nothing at all', async () => {
    const names = readdirSync(join(root, 'api')).filter((f) => f.endsWith('.php'));
    assert.ok(names.length >= 14, `expected the handler set, saw ${names.length}`);
    for (const name of names) {
      const res = await fetch(`http://127.0.0.1:${PORT}/api/${name}`);
      const body = await res.text();
      assert.equal(res.status, 404, `${name} should not be servable`);
      assert.equal(body, '', `${name} leaked ${body.length} bytes`);
    }
  });

  await check('the router itself still answers', async () => {
    const res = await fetch(`http://127.0.0.1:${PORT}/api.php?action=ambient.list`);
    assert.equal(res.status, 401, 'no identity headers -> 401, which proves PHP is alive');
  });
